...
public class MeseService extends CronoService<MeseEntity> {
    ...
    public List<MeseEntity> findAllOrdine() {
        String sortPropertyName = annotationEntityService.getSortProperty(entityClass).orElse(VUOTA);
        Sort sort = annotationListService.getListSort(MeseList.class).orElse(Sort.by(Sort.Direction.ASC, sortPropertyName));
